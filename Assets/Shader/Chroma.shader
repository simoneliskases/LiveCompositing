Shader "Unlit/Transparent Chroma" {
        ...
        SubShader {
                ...
                void surf (Input IN, inout SurfaceOutput o) {
                    /* Sample RGB value of current fragment */
                    half4 c = tex2D (_MainTex, IN.uv_MainTex); 

                    /* Convert sampled RGB value to YCbCr */
					float Y = 0.2989 * c.r + 0.5866 * c.g + 0.1145 * c.b;
 					float Cr = 0.7132 * (c.r - Y);
 					float Cb = 0.5647 * (c.b - Y);
                    
                    /* Convert Mask Color from RGB to YCbCr */
					...
					float maskCr = 0.7132 * (_MaskCol.r - maskY);
 					...
 					float blendValue = smoothstep(_Sensitivity, _Sensitivity + _Smoothing, distance(float2(Cr, Cb), float2(maskCr, maskCb)));
